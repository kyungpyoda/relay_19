...
struct FurnitureOptions: OptionSet {
  ...
  static let 소파 = FurnitureOptions(rawValue: 1 << 1)
  ...
  static let all: [FurnitureOptions] = [.의자, .소파, .침대, .식사테이블, .TV모니터, .전자레인지, .오븐, .냉장고]
  static func makeOptions(set: Array<String>) -> FurnitureOptions {
  ...
  static func makeArray(options: FurnitureOptions) -> [String] {
    self.all.filter { option in
